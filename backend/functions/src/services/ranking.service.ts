		...
		await Promise.all(
			finishedGames.map(async (game) => {
				...
				const winnerGamePoints =
					game.firstTeamPoints > game.secondTeamPoints
				...
				for (const guess of guesses) {
					...
					if (
						guess.firstTeamPoints === game.firstTeamPoints &&
						guess.secondTeamPoints === game.secondTeamPoints
					) {
